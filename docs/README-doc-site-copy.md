...
`hoverTarget` accepts a DOM node, a function that returns a DOM node, or a React ref to an element that you would like to apply the component's [default event handling](#how-it-works) to. If no `hoverTarget` is provided, HoverVideoPlayer will use the component's container `<div>` as the hover target. This prop is useful if you would like a simple way to make the video play when the user hovers over a larger area around the video player.
